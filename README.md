...
The data-generation pipeline itself has no local model dependency, but this repo
also documents a separate fine-tuning and local-inference workflow for Gemma 4
under `kaggle/`.
...
This project publishes two complementary model repositories derived from the
synthetic dataset exported by this repo and fine-tuned in the Kaggle/RunPod
workflow:

- <https://huggingface.co/josoroma/gemma-4-codigo-trabajo-finetune> — merged
  Hugging Face-format checkpoint produced on RunPod after merging the Kaggle
  LoRA adapter into the full Gemma 4 31B model. Use this repo when you want the
  standard Transformers-compatible weights, large safetensor shards, or a base
  for additional conversion/deployment work.
- <https://huggingface.co/josoroma/gemma-4-codigo-trabajo-gguf> — GGUF export
  for local inference workflows. This repo contains the `Q4_K_M` text model and
  the accompanying `BF16-mmproj` artifact generated by Unsloth/llama.cpp for
  Ollama or llama.cpp-based runtimes.

Direct browser links to edit the model cards:

- <https://huggingface.co/josoroma/gemma-4-codigo-trabajo-finetune/new/main?filename=README.md>
- <https://huggingface.co/josoroma/gemma-4-codigo-trabajo-gguf/new/main?filename=README.md>

---

## Kaggle workspace

The `kaggle/` folder contains notebook and experiment artifacts related to model
fine-tuning, merge/export, and publication:
...
1. Build and validate the grounded dataset with Dagster.
2. Fine-tune Gemma 4 31B with Unsloth in the Kaggle notebook.
3. Move the adapter to RunPod A100 80GB for merge and GGUF export.
4. Publish both the merged checkpoint and the GGUF artifacts to Hugging Face.
5. Optionally run the `Q4_K_M` GGUF locally in Ollama.
...
fine-tuning, adapter export, merge, GGUF conversion, and publishing.
...
The notebook driving that workflow is:

- `kaggle/gemma4-31b-text-legal-codigo-de-trabajo.ipynb`

It loads `unsloth/gemma-4-31B-it`, maps the legal JSONL dataset into
conversation format, applies the `gemma-4-thinking` chat template, trains with
`SFTTrainer`, and saves `gemma_4_lora/` as the main training artifact.
...
This repo contains the fully merged safetensor checkpoint generated on RunPod
after reloading the LoRA adapter and calling Unsloth's merged-save path.

Create or reuse the target model repo, then upload with the current HF CLI:

```bash
hf auth login
hf upload josoroma/gemma-4-codigo-trabajo-finetune \
  kaggle/runpod/gemma-4-codigo-trabajo-finetune \
  --repo-type model
```

Typical contents include two large model shards plus tokenizer/config files.

### 3) Publish the GGUF export

Published GGUF model page:

- <https://huggingface.co/josoroma/gemma-4-codigo-trabajo-gguf>

The GGUF export is produced on RunPod after the merged conversion path completes.
One important detail from the working flow: Unsloth writes the final GGUF files
to a suffixed directory such as `gemma-4-codigo-trabajo-gguf_gguf/`, not always
to the original `OUT_PATH` directly.

Upload the GGUF folder with:

```bash
hf auth login
hf upload josoroma/gemma-4-codigo-trabajo-gguf \
  /workspace/gemma-4-codigo-trabajo-gguf_gguf \
  --repo-type model
```

Expected artifacts include:

- `gemma-4-31b-it.Q4_K_M.gguf` — quantized text model for local inference.
- `gemma-4-31b-it.BF16-mmproj.gguf` — multimodal projection artifact generated alongside the model.
- `Modelfile` — starter file for Ollama import.

### 4) Local Ollama usage

The GGUF export was validated locally with Ollama on Apple Silicon using a
model created as `codigo-trabajo-cr`. In practical terms, the flow is:

1. Download the `Q4_K_M` GGUF locally.
2. Build a `Modelfile` that uses an absolute `FROM` path.
3. Run `ollama create codigo-trabajo-cr -f <Modelfile>`.
4. Run `ollama run codigo-trabajo-cr`.

This gives you a local Spanish-language legal assistant backed by the Costa
Rican labor-code fine-tune.
